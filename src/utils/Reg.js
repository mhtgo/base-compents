...
// 字母,数字
export const REG_NUM_CODE = /^[0-9a-zA-Z]+$/
// 银行卡
...
export const PUBLIC_NUM_CODE = (r, v, c) => {
    if (REG_NUM_CODE.test(v) || v === '' || v === undefined || v === null) {
        c()
    } else {
        return c(new Error('只能输入数字或字母'))
    }
}
...
export const PUBLIC_NOT_ID_NUMBER = (r, v, c) => {
    if (v === undefined || v === '' || v === null || REG_ID_NUMBER.test(v)) {
        c()
    } else {
        return c(new Error('请输入正确的身份证号码'))
    }
