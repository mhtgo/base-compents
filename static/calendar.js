window.calendar = (function() {
  ...
  var W = (function() {
    ...
    //初始化dom元素
    function initId(){
      C = C || M(opt.yearSelectId);
      X = X || M(opt.monthSelectId);
      G = G || M(opt.heavenlyId);
      c = c || M(opt.branchId);
      prev = prev || M(opt.prevId);
      next = next || M(opt.nextId);
      Y = Y || M(opt.nowDateId);
  ...
  var N = (function() {
    ...
    function C(data) {
      var Z = Q.getJson();
      var c = Z.dateArray;
      M("cm").style.height = Z.lines * 64 + 2 + "px";
      M("cm").innerHTML = "";
      ...
      for (var a = 0; a < c.length; a++) {
        ...
        X.className = "cell";
        X.style.left = (a % 7) * 84 + "px";
        X.style.top = Math.floor(a / 7) * 64 + 2 + "px";
        ...
        // 是否为星期六日的样式
        //--
        b.style.color = ((a % 7) > 4 || c[a].isRest) ? "#666" : "#666";
        b.innerHTML = c[a].solarDate;
        X.appendChild(b);
        var Y = R("DIV");
        Y.className = "do";
        Y.style.color = "#a62020";
        ...
        if(data){
          for(var b=0; b< data.length; b++){
            ...
            if(addZero(c[a].solarDate) == data[b].date.substring(8)){
                Y.innerHTML = '<b>'+ data[b].noJobPlan + '</b> <span style="float: left; padding-left: 5px;">'+ data[b].wagonNum + 'P </span><span style="float: right; padding-right: 5px;"> ' + data[b].teu  + 'TEU' + '</span>';
            }
          }
        }
  ...
  var defaultOptions = {
    ...
    nowDateId: 'now-date',
    isSelect: true,
    isHeavenlyBranch:false,
    isShowFd: false
  }
  return {
    init: function(options, i, data){
      this.index = i || 0;
...
window.calendar2 = (function() {
  ...
  var N = (function() {
    ...
    function C(data) {
      var Z = Q.getJson();
      var c = Z.dateArray;
      M("cm").style.height = Z.lines * 100 + 2 + "px";
      M("cm").innerHTML = "";
